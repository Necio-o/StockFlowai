...
export const ReminderManager = ({ onAdd, recordatorios, onDelete }: any) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-lg border border-slate-200 dark:border-slate-800">
      <h3 className="text-slate-800 dark:text-white font-bold flex items-center gap-2 mb-4"><Bell size={18}/> RECORDATORIOS</h3>
      <div className="space-y-2">
        <input 
          type="text" placeholder="¿Qué recordar?" 
          className="w-full p-2 rounded bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-white border-none text-sm"
      ...
      <div className="mt-4 max-h-40 overflow-y-auto">
        {recordatorios.map((r: any) => (
          <div key={r.id} className="flex justify-between items-center p-2 border-b border-slate-800 text-[10px]">
            <span className="text-slate-600 dark:text-slate-300">{r.titulo} ({new Date(r.fechaHora).toLocaleString()})</span>
